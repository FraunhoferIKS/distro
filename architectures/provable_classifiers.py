...
import architectures.modules_ibp as modules_ibp


class CNN_IBP(nn.Module):
    def __init__(self, dset_in_name='MNIST', size='L', width=None, last_bias=True, num_classes=None, last_layer_neg=False, normalize=False):
        super().__init__()
    ...
    def ibp_forward(self, l, u):
        ...
        l = self.normalization(l) if self.normalize else l
        u = u.type(torch.get_default_dtype())
        u = self.normalization(u) if self.normalize else u
        for layer in self.layers:
