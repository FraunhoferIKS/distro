...
class JointDiffusionModel(nn.Module):
    def __init__(self, base_model, detector, diffusion, dds_model, normalization=None, classes=None):
        super().__init__()
        assert classes is not None
        self.base_model = base_model
        self.detector = detector
        ...
        if self.normalization is not None:
            self.inv_normalization = torchvision.transforms.Normalize(
    ...
    def forward(self, x, t=None):
        ...
        return torch.logaddexp(log_pi + log_pred, log_po - self.log_classes)
